...
#' @return no data are return to the user, the generated subset is stored on the server side.
#' @author Gaye, A.
#' @export
#' @examples {
#'
...
#' ds.subset(datasources=opals, subset='subD2', data='D', cols=c('DIS_DIAB','PM_BMI_CONTINUOUS'))
#'
...
#' ds.subset(datasources=opals, subset='subD5', data='D', rows=c(1:100))
#' 
#' }
#' 
  ...
  if(is.null(rows) & is.null(cols)){
    if(is.null(logicalOperator) | is.null(threshold)){
      if(!completeCases){
        ...
        cally <- call('subsetDS', dt=data, complt=completeCases)
