...
#' on the server-side. 
#' @details This function is similar to a native R function `seq()`. 
...
#' TO.value.char = "7")`  creates the sequence `3,5,7` on the server-side.\cr
#' (2) `LENGTH.OUT.value.char` indicates the length of the sequence. 
#' For example, `ds.seq(FROM.value.char = "3", BY.value.char = "2", 
#' LENGTH.OUT.value.char = "7")` 
#' creates the sequence `3,5,7,9,11,13,15` on the server-side.\cr
...
#' of length 4 in study 2, the sequence written to study 2 will be `3,5,7,9`.\cr
...
#' that is stored on the data servers. Default `seq.newobj`.  
...
#' @return `ds.seq` returns to the server-side the generated sequence. 
