...
#' @description `ds.glmerSLMA` fits a Generalized Linear Mixed-Effects Model
#' (GLME) on data from one or multiple sources with pooling via SLMA (study-level meta-analysis).
#' @details `ds.glmerSLMA` fits a generalized linear mixed-effects model (GLME) 
...
#'  The SLMA used by default `metafor` package  
...
#'  using `glmer` function can be obtained using R help for `glmer` and the `lme4` package. 
#'  
...
#' It is suggested to make `checks` argument TRUE if an unexplained
...
#'  \describe{
#'    \item{`"binomial"`}{: logistic regression models}
#'    \item{`"poisson"`}{: poisson regression models}
#' }
#' 
#' Note if you are fitting a gaussian model (a standard linear mixed
#' model) you should use `ds.lmerSLMA` and not `ds.glmerSLMA`. 
#' For more information you can see R help for `lmer` and `glmer`. 
#' 
...
#' convergence is declared. The default value for the `check.conv.grad` is `0.001` (note that
#' the default value of this argument in `ds.lmerSLMA` is  `0.002`). 
#' 
#' In `control_value` at present (see `control_type`)
...
#' for `check.conv.grad` is increased from its default value (`0.001`).  Please note
...
#' @param checks logical. If TRUE `ds.glmerSLMA` checks the structural integrity 
...
#' This can be set as `"binomial"` or `"poisson"`. 
#' For more information see **Details**. 
...
#' modified via the `glmerControl` function of the package `lme4`. 
#' For more information see **Details**.  
#' @param control_value numeric representing the new value which you want to allocate the
#' control parameter corresponding to the `control-type`. 
#' For more information see **Details**.
...
#' For more information see  R `glmer` function help. 
...
#' iteratively reweighted least squares (PIRLS) steps. Default `verbose` 
...
#' used only when the argument `assign` is set to TRUE.
#' If no <newobj> argument is specified, the output object defaults to "new.glmer.obj". 
#' @return Many of the elements of the output list returned by `ds.glmerSLMA` are 
#' equivalent to those returned by the `glmer()` function in native R. However,
...
#' @return `offset`: the vector (if any) holding an offset.
...
#' @return `Ntotal`: the total number of observations 
#'                        in the given study (`Nvalid` + `Nmissing`).
