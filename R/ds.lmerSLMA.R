...
#' @details `ds.lmerSLMA` fits a Linear Mixed Effects Model (lme) - can include both fixed and random
...
#' using Study-Level Meta-Analysis (SLMA).  The SLMA used by default `metafor` package  
...
#' LMEs using the `lmer` function can be 
...
#' convergence is declared. The default value for the `check.conv.grad` is `0.002`. 
#' 
#' `control_value` At present (see `control_type`)
...
#' for `check.conv.grad` is increased from its default (`0.002`).  Please note
...
#' tolerance. 
#' 
#' The `optimizer` argument is built in but it won't do anything because there is only one
...
#' For more information see **Details**.
#' @param checks logical. If TRUE `ds.lmerSLMA` checks the structural integrity 
...
#' modified via the `lmerControl` function of the package `lme4`. 
#' For more information see **Details**.  
...
#' For more information see **Details**.
#' @param optimizer specifies the parameter optimizer that `lmer` should use.
#' For more information see **Details**.
...
#' @return Many of the elements of the output list returned by `ds.lmerSLMA` are 
#' equivalent to those returned by the `lmer()` function in native R. However,
...
#' 
#' @return `ds.lmerSLMA` returns a list of  elements mentioned 
