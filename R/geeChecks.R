...
geeChecks <- function(formula, data, datasources){
  ...
  DSI::datashield.assign(datasources, 'Dcomplete', cally)
  ...
  # checks
  colsD <- DSI::datashield.aggregate(datasources, paste0("colnames(", data, ")"))[[1]]
  for(i in 1:length(variables)){
    if(is.na(as.numeric(variables[i], options(warn=-1)))){
      message(paste0("    ", variables[i], "..."))
      for(j in 1: length(datasources)){
        lengthDcomplete <- DSI::datashield.aggregate(datasources[j],paste0("length(Dcomplete)"))[[1]]
        nrowD <- DSI::datashield.aggregate(datasources[j], paste0("dim(", data, ")"))[[1]][1]
