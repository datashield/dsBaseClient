...
#' `ds.make(toAssign = '5.3 + beta*xvar', newobj = 'predvals')`. 
#' This is a typical call you
#' may make to get the predicted values from a simple linear regression model where
#' a `y` variable is regressed against an `x` variable (`xvar`) 
#' where the estimated regression intercept is `5.3` 
#' and `beta` is the estimated regression slope. 
#' 
#' This call appears to
#' fail because in interpreting the arithmetic function which is its first argument
#' it first encounters the (length 1) scalar `5.3` 
#' and when it then encounters the `xvar` vector
...
#' to create a vector
#' of length equal to `xvar` with each value equal to `5.3`. 
#' 
#' There are two work-around solutions here: 
#' 
#' (1) explicitly create a vector of appropriate length with each
#' value equal to `5.3`. To do this there is a useful trick. First identify
...
#' `ds.make(toAssign = 'indID-indID+1',newobj = 'ONES')`. This creates a vector of ones (called `ONES`)
#' in each source equal in length to the `indID` vector in that source. Then issue
#' the second call `ds.make(toAssign = 'ONES*5.3',newobj = 'vect5.3')` which creates the required
...
#' `ds.make(toAssign = '(beta*xvar)+5.3', newobj = 'predvals')`
...
#' `ds.make(toAssign = 'indID-indID',newobj = 'ZEROS')` 
...
#' variable that is stored on the data servers. Default `make.newobj`. 
...
#' @return `ds.make` returns the new object which is written to the
