...
test_that("combine - performance", {
    glmSLMA.res <- ds.glmSLMA("D$LAB_TSC~D$LAB_TRIG", family = "gaussian", newobj = "perf.glm.obj")

    expect_equal(glmSLMA.res$num.valid.studies, 3)
    expect_equal(glmSLMA.res$validity.check, "<perf.glm.obj> appears valid in all sources")
    ...
    while (difftime(.current.time, .start.time, units = "secs")[[1]] < .durationSec) {
        ds.glmPredict("perf.glm.obj", newdataname = NULL, output.type = "response", se.fit = FALSE, na.action = "na.pass")
