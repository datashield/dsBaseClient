...
test_that("combine - performance", {
    ...
    while (difftime(.current.time, .start.time, units = "secs")[[1]] < .durationSec) {
        ds.rbind(x = c("D$LAB_TSC", "D$LAB_TRIG"), newobj = "perf.newobj")
    ...
    .reference.rate <- perf.reference.rate("ds.rbind::perf::0")
    if (any(length(.reference.rate) == 0) || any(is.null(.reference.rate))) {
        print(paste("ds.rbind::perf::0 ", .current.rate, 0.5, 2.0))
        perf.reference.save("ds.rbind::perf::0", .current.rate, 0.5, 2.0)
    } else {
        print(paste("ds.rbind::perf::0 ", format(.current.rate, digits = 8), ", ", format(100.0 * .current.rate / .reference.rate, digits = 4), "%", sep = ''))
    }

    .reference.rate            <- perf.reference.rate("ds.rbind::perf::0")
    .reference.tolerance.lower <- perf.reference.tolerance.lower("ds.rbind::perf::0")
    .reference.tolerance.upper <- perf.reference.tolerance.upper("ds.rbind::perf::0")
