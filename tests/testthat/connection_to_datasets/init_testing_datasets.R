...
init.testing.datasets <- function()
{
  ...
  if (ds.test_env$secure_login_details)
  {
    ...
    ds.test_env$local.values          <- list(ds.test_env$local.values.1,ds.test_env$local.values.2,ds.test_env$local.values.3)
    ds.test_env$combined.local.values <- rbind(ds.test_env$local.values.1,ds.test_env$local.values.2,ds.test_env$local.values.3)
...
init.dataset.3 <- function()
{
  ...
  if (ds.test_env$secure_login_details)
  {
    local.values.3.name <- load("data_files/TESTING/DATASET3.rda")
    ds.test_env$local.values.3 <- eval(as.symbol(local.values.3.name))
    ds.test_env$local.values   <- list(ds.test_env$local.values.3)
    if (ds.test_env$driver == "OpalDriver")
    {
      builder <- DSI::newDSLoginBuilder(.silent = TRUE)
      builder$append(server = "study3", url = ds.test_env$ip_address_3, user = ds.test_env$user_3, password = ds.test_env$password_3, table = "TESTING.DATASET3")
...
init.dataset.2 <- function()
{
  ...
  if (ds.test_env$secure_login_details)
  {
    local.values.2.name <- load("data_files/TESTING/DATASET2.rda")
    ds.test_env$local.values.2 <- eval(as.symbol(local.values.2.name))
    ds.test_env$local.values   <- list(ds.test_env$local.values.2)
    if (ds.test_env$driver == "OpalDriver")
    {
      builder <- DSI::newDSLoginBuilder(.silent = TRUE)
      builder$append(server = "study2", url = ds.test_env$ip_address_2, user = ds.test_env$user_2, password = ds.test_env$password_2, table = "TESTING.DATASET2")
...
init.dataset.1 <- function()
{
  ...
  if (ds.test_env$secure_login_details)
  {
    local.values.1.name <- load("data_files/TESTING/DATASET1.rda")
    ds.test_env$local.values.1 <- eval(as.symbol(local.values.1.name))
    ds.test_env$local.values   <- list(ds.test_env$local.values.1)
    if (ds.test_env$driver == "OpalDriver")
    {
      builder <- DSI::newDSLoginBuilder(.silent = TRUE)
      builder$append(server = "study1", url = ds.test_env$ip_address_1, user = ds.test_env$user_1, password = ds.test_env$password_1, table = "TESTING.DATASET1")
...
init.testing.dataset.factor_levels <- function()
{
  ...
  if (ds.test_env$secure_login_details)
  {
    local.values.1.name <- load("data_files/FACTOR_LEVELS/FACTOR_LEVELS_DATASET1.rda")
    ds.test_env$local.values.1 <- eval(as.symbol(local.values.1.name))
    local.values.2.name <- load("data_files/FACTOR_LEVELS/FACTOR_LEVELS_DATASET2.rda")
    ds.test_env$local.values.2 <- eval(as.symbol(local.values.2.name))
    local.values.3.name <- load("data_files/FACTOR_LEVELS/FACTOR_LEVELS_DATASET3.rda")
    ds.test_env$local.values.3 <- eval(as.symbol(local.values.3.name))
    ds.test_env$local.values   <- list(ds.test_env$local.values.1,ds.test_env$local.values.2,ds.test_env$local.values.3)
    if (ds.test_env$driver == "OpalDriver")
    {
