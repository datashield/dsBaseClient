...
connect.studies.dataset.anthro(list("sex", "weight", "height"))
...
test_that("combine - performance", {
    ...
    while (difftime(.current.time, .start.time, units = "secs")[[1]] < .durationSec) {
        ds.getWGSR("D$sex", "D$weight", "D$height", "wfh", newobj = "perf.newobj")
    ...
    .reference.rate <- perf.reference.rate("ds.getWGSR::perf::0")
    if (any(length(.reference.rate) == 0) || any(is.null(.reference.rate))) {
        print(paste("ds.getWGSR::perf::0 ", .current.rate, 0.5, 2.0))
        perf.reference.save("ds.getWGSR::perf::0", .current.rate, 0.5, 2.0)
    } else {
        print(paste("ds.getWGSR::perf::0 ", format(.current.rate, digits = 8), ", ", format(100.0 * .current.rate / .reference.rate, digits = 4), "%", sep = ''))
    }

    .reference.rate            <- perf.reference.rate("ds.getWGSR::perf::0")
    .reference.tolerance.lower <- perf.reference.tolerance.lower("ds.getWGSR::perf::0")
    .reference.tolerance.upper <- perf.reference.tolerance.upper("ds.getWGSR::perf::0")
