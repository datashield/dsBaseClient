...
ds.asFactor(input.var.name = "D$GENDER", newobj.name = "gender.factor")

#
# Tests
#

# context("ds.levels::perf:0")
test_that("combine - performance", {
    ...
    .reference.rate <- perf.reference.rate("ds.levels::perf::0")
    if (any(length(.reference.rate) == 0) || any(is.null(.reference.rate))) {
        print(paste("ds.levels::perf::0 ", .current.rate, 0.5, 2.0))
        perf.reference.save("ds.levels::perf::0", .current.rate, 0.5, 2.0)
    } else {
        print(paste("ds.levels::perf::0 ", format(.current.rate, digits = 8), ", ", format(100.0 * .current.rate / .reference.rate, digits = 4), "%", sep = ''))
    }

    .reference.rate            <- perf.reference.rate("ds.levels::perf::0")
    .reference.tolerance.lower <- perf.reference.tolerance.lower("ds.levels::perf::0")
    .reference.tolerance.upper <- perf.reference.tolerance.upper("ds.levels::perf::0")
