...
test_that("combine - performance", {
    ...
    if (any(length(.reference.rate) == 0) || any(is.null(.reference.rate))) {
        ...
        print(paste("ds.mean::perf::combine:0 ", format(.current.rate, digits = 8), ",", format(.current.rate / .reference.rate, digits = 4), "%", sep = ''))
    }
...
test_that("split - performance", {
    ...
    if (any(length(.reference.rate) == 0) || any(is.null(.reference.rate))) {
        ...
        print(paste("ds.mean::perf::split:0 ", format(.current.rate, digits = 8), ",", format(.current.rate / .reference.rate, digits = 4), "%", sep = ''))
    }
