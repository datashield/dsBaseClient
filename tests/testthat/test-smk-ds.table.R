...
test_that("simple table 3D", {
  ...
  expect_equal(res.errors$study1, "Execution failed: Error : Failed: at least one cell has a non-zero count less than nfilter.tab i.e. 3", fixed = TRUE)
  expect_equal(res.errors$study2, "Execution failed: Error : Failed: at least one cell has a non-zero count less than nfilter.tab i.e. 3", fixed = TRUE)
})
