...
Computes the square root values for a specified numeric or integer
vector. This function is similar to R function `sqrt`.
...
- newobj:
  ...
  is stored on the data servers. Default name is set to `sqrt.newobj`.
...
`ds.sqrt` assigns a vector for each study that includes the square root
...
square root values of the elements of a numeric or integer vector and
assigns a new vector with those square root values on the server-side.
The name of the new generated vector is specified by the user through
the argument `newobj`, otherwise is named by default to `sqrt.newobj`.
