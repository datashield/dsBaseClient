...
which is written to the server-side. Also, two validity messages are
returned to the client-side indicating the name of the `newobj` that has
been created in each data source and if it is in a valid form.
